...
import com.donghk.core.interceptor.DbInterceptor;
...
public class PageForJqGrid<T> {
	...
	private int rows = 10; // 每页显示数量
	private String sidx = null; // 排序字段
	private String sord = null; // 排序规则
	private boolean autoCount = true;

	private List<T> result = new ArrayList<T>();
	private long total = -1L; // 总页数
	private long records = -1L; // 总记录数
	...
	public void setSord(String sord) {
		...
		for (String sordStr : sords) {
			if ((!StringUtils.equals(PageForJqGrid.DESC, sordStr)) && (!StringUtils.equals(PageForJqGrid.ASC, sordStr))) {
	...
	public boolean isSidxSetted() {
		return (StringUtils.isNotBlank(this.sidx)) && (StringUtils.isNotBlank(this.sord));
	}
	 ...
	 * @return
	 */
	public Map<String, Object> pageToMap(PageForJqGrid<T> page, Map<String, Object> map, Integer startIndex) {
		if (!EmptyUtil.isEmpty(startIndex) && !EmptyUtil.isEmpty(page.getRows())) {
	...
	public PageForJqGrid<T> listToPage(PageForJqGrid<T> page, List<T> list) {
		page.setRecords(DbInterceptor.getResultCount());
		page.setResult(list);
